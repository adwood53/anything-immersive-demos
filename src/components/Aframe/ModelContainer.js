...
function ModelContainer({ config = {} }) {
  ...
  useEffect(() => {
    ...
    if (modelEntity) {
      modelEntity.setAttribute('model-control', '');
      modelEntity.setAttribute('animation-mixer-control', '');
      modelEntity.setAttribute('disable-culling', '');
      if (format === 'obj') {
  ...
  }, [config, assetId, format, modelId, mtlAssetId]);
