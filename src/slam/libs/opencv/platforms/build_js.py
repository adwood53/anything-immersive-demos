...
class Builder:
    ...
    def get_cmake_cmd(self):
        cmd = [
            ...
            "-DPYTHON_DEFAULT_EXECUTABLE=%s" % sys.executable,
               ...
               "-DCMAKE_TOOLCHAIN_FILE=%s" % self.get_toolchain_file().replace('\\', '/'),
               "-DCPU_BASELINE=''",
    ...
    def build_opencvjs(self):
        execute(["ninja", "-j", str(multiprocessing.cpu_count()), "opencv.js"])

    def build_test(self):
        execute(["ninja", "-j", str(multiprocessing.cpu_count()), "opencv_js_test"])

    def build_perf(self):
        execute(["ninja", "-j", str(multiprocessing.cpu_count()), "opencv_js_perf"])

    def build_doc(self):
        execute(["ninja", "-j", str(multiprocessing.cpu_count()), "doxygen"])

    def build_loader(self):
        execute(["ninja", "-j", str(multiprocessing.cpu_count()), "opencv_js_loader"])
